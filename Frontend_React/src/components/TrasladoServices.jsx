...
import { User, Users, Briefcase, Plane, Check, Car, Shield, Clock, Headphones, ArrowRight, ShoppingBag } from 'lucide-react';
import api from '../api/axios';
import { useCart } from '../context/CartContext';

export function TrasladoServices() {
  ...
  const handleAddToCart = async (serviceId) => {
    ...
    if (res.success) {
      setToast({ show: true, message: '¡Traslado agregado al carrito!', type: 'success' });
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        //  el mapeo de los servicios para que se muestren en las tarjetas
        const filtered = data.filter(item => item.tipo_servicio === 'transporte');
        setServices(filtered);
        setLoading(false);
      } catch (err) {
        console.error('Error al obtener servicios de transporte:', err);
        setError('No se pudieron cargar los servicios de transporte desde la API.');
        setLoading(false);
      }
    };
  ...
  //  la funcion que mapea los datos de los servicios
  const getMappedCardData = (service) => {
    const nameLower = service.nombre.toLowerCase();

    if (nameLower.includes('grupo')) {
      return {
        title: 'Traslados Grupales',
        description: 'Movilizamos grupos grandes con comodidad y seguridad. Ideal para tours, eventos y viajes corporativos.',
        features: ['Minivans y buses', 'Capacidad personalizada', 'Viajes nacionales'],
        icon: Users,
        image: 'https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?auto=format&fit=crop&q=80&w=600'
      };
    }

    if (nameLower.includes('empresa') || nameLower.includes('corporativo')) {
      return {
        title: 'Traslados para Empresas',
        description: 'Soluciones de transporte corporativo eficiente y confiable para ejecutivos y colaboradores.',
        features: ['Cuentas corporativas', 'Facturación electrónica', 'Servicio recurrente'],
        icon: Briefcase,
        image: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=600'
      };
    }

    if (nameLower.includes('aeropuerto') || nameLower.includes('airport')) {
      return {
        title: 'Traslados Aeropuerto',
        description: 'Te esperamos o te llevamos al aeropuerto con puntualidad y seguimiento de vuelos.',
        features: ['Monitoreo de vuelos', 'Sin cargos por retrasos', 'Disponibilidad 24/7'],
        icon: Plane,
    ...
    //  traslado individual
    return {
      title: 'Traslados Ejecutivos Individuales',
      description: 'Viajes privados y personalizados en vehículos de alta gama. Ideal para viajeros exigentes que buscan confort y discreción.',
      features: ['Vehículos de lujo', 'Choferes profesionales', 'Servicio puerta a puerta'],
      icon: User,
  ...
  return (
    <section className="py-16 bg-[#fcfcfc] border-t border-neutral-100" style={{ fontFamily: "'Outfit', sans-serif" }}>
      <div className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-8">
        ...
        <div className="mb-12">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#062e20] tracking-tight">
            Nuestros servicios de traslados
          </h2>
          <br></br>
          ...
          <p className="text-neutral-500 text-sm sm:text-base mt-2 max-w-3xl font-light leading-relaxed">
            Contamos con una flota moderna y conductores profesionales para brindarte la mejor experiencia de transporte en cada viaje.
          </p>
        </div>
        ...
        {/* state cuando no hay servicios */}
        {!loading && !error && services.length === 0 && (
          <div className="text-center text-neutral-500 py-12 mb-16">
            No hay servicios de traslado (transporte) disponibles en la base de datos.
          </div>
        )}
        ...
        {!loading && !error && services.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {services.map((service) => {
              const card = getMappedCardData(service);
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-48 w-full overflow-hidden">
                    <img
                      src={service.imagen_url || 'http://127.0.0.1:8000/storage/servicios/no%20hay%20imagen.webp'}
                      alt={service.nombre}
                  ...
                  <div className="px-6 pb-6 pt-8 flex-grow flex flex-col justify-between relative">
                    ...
                    {/* icono flotante circular de la tarjeta  */}
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <h3 className="text-lg font-bold text-[#062e20] leading-snug group-hover:text-[#0d5638] transition-colors duration-300">
                        {service.nombre !== 'Vuelo Madrid-Roma' && service.nombre !== 'traslado al aeropuerto' && service.nombre !== 'traslado al aeropuerto GRUPO' && service.nombre !== 'traslado para empresas'
                          ? service.nombre
                          : card.title}
                      </h3>
                      {/* descripción de la tarjeta  */}
                      <p className="text-xs sm:text-sm text-neutral-500 leading-relaxed font-light line-clamp-3">
                        {service.descripcion && service.descripcion !== 'aeropuerto' && service.descripcion !== 'Vuelo directo ida y vuelta'
                          ? service.descripcion
                          : card.description}
                      </p>
                      ...
                      <ul className="text-left space-y-2 pt-2 pb-4">
                        {card.features.map((feature, index) => (
                          <li key={index} className="flex items-center gap-2 text-xs text-neutral-600 font-medium">
                    ...
                    {/* precio y boton de añadir al carrito */}
        ...
        <div className="bg-[#f7f9f7] rounded-3xl p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16 border border-neutral-100">
          ...
          <div className="flex gap-4 items-start border-b sm:border-b-0 pb-4 sm:pb-0 sm:border-r border-neutral-200/60 last:border-0 pr-4">
            ...
            <div>
              <h4 className="font-bold text-[#062e20] text-sm sm:text-base">Flota moderna</h4>
              <p className="text-neutral-500 text-xs sm:text-sm font-light mt-0.5">Vehículos confortables y equipados.</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-start border-b sm:border-b-0 pb-4 sm:pb-0 lg:border-r border-neutral-200/60 last:border-0 pr-4">
            ...
            <div>
              <h4 className="font-bold text-[#062e20] text-sm sm:text-base">Viajes seguros</h4>
              <p className="text-neutral-500 text-xs sm:text-sm font-light mt-0.5">Conductores profesionales y protocolos de seguridad.</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-start border-b sm:border-b-0 pb-4 sm:pb-0 sm:border-r border-neutral-200/60 last:border-0 pr-4">
            ...
            <div>
              <h4 className="font-bold text-[#062e20] text-sm sm:text-base">Puntualidad garantizada</h4>
              <p className="text-neutral-500 text-xs sm:text-sm font-light mt-0.5">Cumplimos con los horarios acordados.</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-start last:border-0">
            ...
            <div>
              <h4 className="font-bold text-[#062e20] text-sm sm:text-base">Atención personalizada</h4>
              <p className="text-neutral-500 text-xs sm:text-sm font-light mt-0.5">Te asesoramos en cada detalle de tu viaje.</p>
            </div>
          </div>
        ...
        <div className="relative rounded-3xl overflow-hidden bg-[#051a14] text-white p-8 sm:p-12 lg:p-16 flex flex-col lg:flex-row items-center justify-between gap-8 shadow-lg">
          ...
          <div
            ...
            style={{ backgroundImage: "url('https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?auto=format&fit=crop&q=80&w=1200')" }}
          />
          ...
          <div className="relative z-10 max-w-xl space-y-4 text-center lg:text-left">
            ...
            <p className="text-neutral-300 text-sm sm:text-base font-light">
              Cotiza tu traslado y recibe la mejor opción según tus necesidades.
            </p>
          </div>
        ...
        {/* notificacion de añadir productos al carrito o al login (toast notification)*/}
        {toast.show && (
